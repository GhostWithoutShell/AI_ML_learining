...
class IDMDiscriminator(nn.Module):
    def __init__(self, input_nc=1, ndf=64):
        ...
        # 1. Впрыскиваем хаос (шум), чтобы дискриминатор не переобучался
        self.noise = GaussianNoise(sigma=0.1)

        # 2. Сверточный слой (Spectral Norm держит веса в узде)
        self.main = nn.Sequential(
            ...
            # Выходной слой (PatchGAN output)
            # Мы НЕ используем Sigmoid здесь, так как будем использовать BCEWithLogitsLoss
            # Выход будет матрицей, а не одним числом!
